...
// WO-24: Tests for config path discovery precedence
...
func TestDiscover_XDG(t *testing.T) {
	...
	// - Windows: %AppData%
	// We use a temp dir and override via XDG_CONFIG_HOME (Linux only).
	// On macOS/Windows, we skip this test if the override doesn't work.
	_ = os.Unsetenv("DNSSPECTRE_CONFIG")

	// Create temp dir and override XDG_CONFIG_HOME
	tmpDir := t.TempDir()
	_ = os.Setenv("XDG_CONFIG_HOME", tmpDir)
	defer func() { _ = os.Unsetenv("XDG_CONFIG_HOME") }()

	// Check if the override worked (Linux)
	configHome, err := os.UserConfigDir()
	if err != nil {
		t.Skip("UserConfigDir not available")
	}
	if configHome != tmpDir {
		// Override didn't work (macOS/Windows) - skip to avoid touching real config dir
		t.Skip("XDG_CONFIG_HOME override not supported on this platform")
	}
